...
import "./PoolAccess-style.scss";

// For global access
declare global {
    interface Window {
        orgJDrupesVmOperatorPoolAccess: { 
...
window.orgJDrupesVmOperatorPoolAccess = {};
...
window.orgJDrupesVmOperatorPoolAccess.initPreview = (previewDom: HTMLElement,
    _isUpdate: boolean) => {
    const app = createApp({
        setup(_props: object) {
          ...
          <table>
            <tbody>
              <tr>
                ...
                <td class="jdrupes-vmoperator-poolaccess-preview-action-list">
                  <span role="button" 
...
JGConsole.registerConletFunction("org.jdrupes.vmoperator.poolaccess.PoolAccess",
    "updateConfig",
    function(conletId: string, type: string, resource: string) {
        ...
        const api = getApi<Api>(conlet.element().querySelector(
            ":scope .jdrupes-vmoperator-poolaccess-preview"))!;
        if (type === "VM") {
            api.vmName = resource;
            api.poolName = "";
        } else {
...
JGConsole.registerConletFunction("org.jdrupes.vmoperator.poolaccess.PoolAccess",
    "openConsole", function(_conletId: string, mimeType: string, data: string) {
        let target = document.getElementById(
            "org.jdrupes.vmoperator.poolaccess.PoolAccess.target");
        if (!target) {
            target = document.createElement("iframe");
            target.id = "org.jdrupes.vmoperator.poolaccess.PoolAccess.target";
...
window.orgJDrupesVmOperatorPoolAccess.initEdit = (dialogDom: HTMLElement,
    isUpdate: boolean) => {
    if (isUpdate) {
        return;
    }
...
window.orgJDrupesVmOperatorPoolAccess.applyEdit =
...
window.orgJDrupesVmOperatorPoolAccess.confirmReset = 
